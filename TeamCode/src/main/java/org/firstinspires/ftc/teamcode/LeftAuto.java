...
public class LeftAuto extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (System.currentTimeMillis() - startTime < LOOP_TIME && (!filenames.isEmpty() || !steps.isEmpty())) {
            ...
            if (!(verticalBucket.getTarget() == 0 && verticalBucket.getCurrentPosition() > 0)) {
                verticalBucket.moveToTarget(0.5, 0.1, true);
            } else if (verticalBucket.getTarget() == 0 && verticalBucket.getCurrentPosition() > 0) {
                verticalBucket.setPower(-0.00);
            } else {
                verticalBucket.setPower(0.0);
            }

            if (verticalArmMotor.checkTarget(125) || steps.get(0).lift_tolerance == 1.0) { liftCheck = true; }
            if (horizontalArmMotor.checkTarget(100) || steps.get(0).arm_tolerance == 1.0) { armCheck = true; }

            drive.setTargetPose(steps.get(0).getPose());
            poseCheck = steps.get(0).pos_tolerance == 1.0 || drive.posPIDMecanumDrive(odometry.getPose(), steps.get(0).pos_tolerance, steps.get(0).angle_tolerance, steps.get(0).drive_max, true);
            ...
            telemetry.addData("Bucket Pos: ", verticalBucket.getCurrentPosition());
            telemetry.addData("Bucket Target: ", verticalBucket.getTarget());
            ...
            if (System.currentTimeMillis() - saveTime >= steps.get(0).millis && (System.currentTimeMillis() - saveTime) >= 5000.0) {
                steps.remove(0);
                saveTime = System.currentTimeMillis();
            }
